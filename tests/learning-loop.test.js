...
test('三星 Chrome 的懸停 button=1 與 contextmenu 可切換筆和橡皮擦', () => {
  const { run } = loadApp();
  const result = plain(run(`(() => {
    const active = { pen:false, erase:false }, status = { textContent:'' };
    const button = (key) => ({ classList:{ toggle(name, value){ if (name === 'active') active[key] = value; } } });
    ...
    const first = { mode:paperSourceSession.inkMode, shown:canvas.dataset.mode, status:status.textContent, active:{ ...active } };
    paperInkMove(hover(1));
    const held = paperSourceSession.inkMode;
    paperInkMove(hover(-1));
    paperInkMove(hover(1));
    const second = { mode:paperSourceSession.inkMode, status:status.textContent };
    paperSourceSession.sPenContextIgnoreUntil = 0; paperSourceSession.sPenLastAt = Date.now();
    let prevented = false;
    paperInkContextMenu({ preventDefault(){ prevented = true; } });
    const menu = { mode:paperSourceSession.inkMode, status:status.textContent, prevented };
    return { first, held, second, menu };
  })()`));
  assert.deepEqual(result.first, { mode:'erase', shown:'erase', status:'S Pen 側鍵：已切換為橡皮擦', active:{ pen:false, erase:true } });
  assert.equal(result.held, 'erase');
  assert.deepEqual(result.second, { mode:'pen', status:'S Pen 側鍵：已切換為筆' });
  assert.deepEqual(result.menu, { mode:'erase', status:'S Pen 側鍵：已切換為橡皮擦', prevented:true });
});
