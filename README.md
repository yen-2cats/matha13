...
目前版本為 `0716e`。全站已重做為暖白、石墨字與低彩度灰褐／橄欖色的簡潔介面；導覽與狀態圖示使用專案內建的 SVG，不以 emoji 當正式 UI。核心題庫已由 82 題補到 362 題，首頁以完整模擬校準 9→13 級分，並依到期錯題、校準需求與弱項產生唯一的下一步。
...
3. Supabase：登入後的跨裝置狀態、內容包與手寫歸檔；所有個人資料表都必須開 RLS。

## 安全與資料規則

- 公開 repo 只能放可公開的程式與內建內容；付費講義、官方受限內容、帳密、營運手冊與 service-role key 禁止提交。
- Supabase publishable key 可放前端；service-role key 絕對不可放前端。
- OpenAI API key 只保存在可管理的 Supabase 專案 `rrihysbxhsbxjteqmtdu` 的 Edge Function Secret；瀏覽器、`app_state`、localStorage 與備份都不保存 Key。AI 代理會以原數A Supabase 登入權杖驗證使用者，未登入請求回傳 401。
